...
Cursor cursor_insert_str(EditorState *editor, Cursor cursor, char *buf,
                         int len) {
...
void editor_splice_into_line(EditorState *editor, int line, int col, char *s, int len) {
  assert(col >= 0);
...
void mu_editor(mu_Context *ctx, EventState *event, EditorState *editor,
  ...
  if (focused) {
    cursor = cursor_insert_str(editor, cursor, event->input_text,
                               strlen(event->input_text));
	...
	// If we attach the space back on itself twisted (ala mobius strip),
